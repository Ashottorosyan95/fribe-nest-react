...
import { UserValidatePublishableKeyDto } from '../dtos/user.validate-publishable-key.dto';
...
export class UserPublicController {
    ...
    @Post('/validatePublishableKey')
    async validatePublishableKey(
        @Body()
        { publishableKey, serviceType }: UserValidatePublishableKeyDto
    ): Promise<any> {
    ...
    @Post('/login')
    async login(@Body() { email, password, role }: UserLoginDto): Promise<IResponse> {
        ...
        if (role) {
            if (userWithRole.role.type === ENUM_ROLE_TYPE.USER) {
                throw new NotFoundException({
                    statusCode: ENUM_USER_STATUS_CODE_ERROR.NOT_ADMIN_ERROR,
            ...
            if ((userWithRole.role.type === ENUM_ROLE_TYPE.SUPER_ADMIN || userWithRole.role.type === ENUM_ROLE_TYPE.ADMIN)) {
                throw new NotFoundException({
                    statusCode: ENUM_USER_STATUS_CODE_ERROR.NOT_USER_ERROR,
    ...
    async signUp(
        ...
        const [role, emailExist, apiKey, mobileNumberExist] = await Promise.all(
            promises
        );
