...
import { UserServicesUsageService } from 'src/common/user-services-usage/services/user-services-usage.service';
...
export class UserAdminController {
    constructor(
        ...
        private readonly userServicesUsageService: UserServicesUsageService
    ) { }
    ...
    async list(
        ...
        const usersWithUsageCount = await Promise.all(
    ...
    @Get('/get-by-access-token/:accesstoken')
    async getUserByAccessToken(@Param('accesstoken') accesstoken: string): Promise<IResponse> {
    ...
    // @AuthJwtAdminAccessProtected()
    // @RequestParamGuard(UserRequestDto)
    @Patch('/update/:id/inactive')
    async inactive(@Param('id') id): Promise<IResponse> {
        try {
            const user: UserDoc = await this.userService.findOneById(id);
    ...
    // @AuthJwtAdminAccessProtected()
    // @RequestParamGuard(UserRequestDto)
    @Patch('/update/:id/active')
    async active(@Param('id') id): Promise<IResponse> {
        try {
            const user: UserDoc = await this.userService.findOneById(id);
