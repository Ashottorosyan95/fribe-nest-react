...
const user_validate_publishable_key_dto_1 = require("../dtos/user.validate-publishable-key.dto");
const role_enum_constant_1 = require("../../role/constants/role.enum.constant");
let UserPublicController = exports.UserPublicController = class UserPublicController {
    constructor(databaseConnection, userService, authService, roleService, settingService, apiKeyService) {
    ...
    async validatePublishableKey({ publishableKey, serviceType }) {
        return this.authService.validatePublishableKey(publishableKey, serviceType);
    }
    async login({ email, password, role }) {
        ...
        if (role) {
            if (userWithRole.role.type === role_enum_constant_1.ENUM_ROLE_TYPE.USER) {
                throw new common_1.NotFoundException({
                    statusCode: user_status_code_constant_1.ENUM_USER_STATUS_CODE_ERROR.NOT_ADMIN_ERROR,
                    message: 'user.error.notAdmin',
                });
            }
            else {
                return {
            ...
            if ((userWithRole.role.type === role_enum_constant_1.ENUM_ROLE_TYPE.SUPER_ADMIN || userWithRole.role.type === role_enum_constant_1.ENUM_ROLE_TYPE.ADMIN)) {
                throw new common_1.NotFoundException({
                    statusCode: user_status_code_constant_1.ENUM_USER_STATUS_CODE_ERROR.NOT_USER_ERROR,
                    message: 'user.error.notUser',
                });
            }
            else {
                return {
    ...
    async signUp({ email, mobileNumber, ...body }) {
        const shortId = (0, ulid_1.ulid)();
        const clientId = (0, ulid_1.ulid)();
        const publishableKey = (0, ulid_1.ulid)();
        ...
        const promises = [
            ...
            this.apiKeyService.createHashApiKey(key, secret)
        ];
        if (mobileNumber) {
            promises.push(this.userService.existByMobileNumber(mobileNumber));
        }
        const [role, emailExist, apiKey, mobileNumberExist] = await Promise.all(promises);
        if (emailExist) {
    ...
    async signUpGoogleCallback({ email, firstName, lastName, accessToken: googleAccessToken, refreshToken: googleRefreshToken, }) {
        ...
        try {
            ...
            const user = await this.userService.create({
                ...
                companyName: 'Cremin - Senger',
                role: role._id,
                shortId: (0, ulid_1.ulid)(),
                clientId: (0, ulid_1.ulid)(),
                publishableKey: (0, ulid_1.ulid)(),
                apiKey: '',
...
__decorate([
    (0, common_1.Post)('/validatePublishableKey'),
    openapi.ApiResponse({ status: 201, type: Object }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [user_validate_publishable_key_dto_1.UserValidatePublishableKeyDto]),
    __metadata("design:returntype", Promise)
], UserPublicController.prototype, "validatePublishableKey", null);
__decorate([
