...
const user_services_usage_service_1 = require("../../../common/user-services-usage/services/user-services-usage.service");
let UserAdminController = exports.UserAdminController = class UserAdminController {
    constructor(authService, paginationService, userService, roleService, userServicesUsageService) {
    ...
    async list({ _search, _limit, _offset, _order }, isActive, blocked, inactivePermanent) {
        const roles = await this.roleService.findAll({ type: 'USER' });
        ...
        const usersWithUsageCount = await Promise.all(users.map(async (user) => {
            const usageCount = await this.userServicesUsageService.findAll({ userId: user._id });
            return { ...user, usageReq: usageCount.length };
        }));
        const total = await this.userService.getTotal(find);
        const totalPage = this.paginationService.totalPage(total, _limit);
        return {
            _pagination: { total, totalPage },
            data: usersWithUsageCount,
        };
    }
    async get(user) {
    ...
    async getUserByAccessToken(accesstoken) {
        let userInfo = await this.authService.payloadAccessToken(accesstoken);
        userInfo = {
            ...userInfo,
            data: {
    ...
    async inactive(id) {
        try {
            ...
            const update = await this.userService.inactive(user);
            return {
                data: {
                    _id: update._id,
    ...
    async active(id) {
        try {
            const user = await this.userService.findOneById(id);
            const update = await this.userService.active(user);
            return {
                data: {
                    _id: update._id,
                    isActive: update.isActive
...
__decorate([
    ...
    (0, common_1.Get)('/get-by-access-token/:accesstoken'),
    openapi.ApiResponse({ status: 200, type: Object }),
    __param(0, (0, common_1.Param)('accesstoken')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], UserAdminController.prototype, "getUserByAccessToken", null);
__decorate([
...
__decorate([
    ...
    (0, common_1.Patch)('/update/:id/inactive'),
    openapi.ApiResponse({ status: 200, type: Object }),
...
__decorate([
    ...
    (0, common_1.Patch)('/update/:id/active'),
    openapi.ApiResponse({ status: 200, type: Object }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
...
exports.UserAdminController = UserAdminController = __decorate([
    ...
    __metadata("design:paramtypes", [auth_service_1.AuthService,
        ...
        user_services_usage_service_1.UserServicesUsageService])
], UserAdminController);
