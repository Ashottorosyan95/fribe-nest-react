...
export function AddLocation(): MethodDecorator {
    return applyDecorators(
        Doc<any>('place.addLocation', {
            auth: {
                jwtAccessToken: true,
                // permissionToken: true,
            },
...
export function TextSearchDoc(): MethodDecorator {
    return applyDecorators(
        Doc<PlaceGetSerialization>('place.textsearch', {
            auth: {
                jwtAccessToken: true,
                // permissionToken: true,
            },
            request: {
                queries: [...PlaceDocQueryTextSearch],
            },
            response: { serialization: PlaceListSerialization },
        })
    );
...
export function GetLocationWithLatLongDoc(): MethodDecorator {
    return applyDecorators(
        Doc<PlaceGetSerialization>('place.GetLocationLatLong', {
            auth: {
                jwtAccessToken: true,
                // permissionToken: true,
            },
            request: {
                queries: [...GetLocationWithLatLongQuery],
            },
            response: { },
        })
    );
